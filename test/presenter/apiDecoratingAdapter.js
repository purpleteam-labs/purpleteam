...
describe('apiDecoratingAdapter', () => {
  ...
  describe('postToApi', () => {
    ...
    it('- on - invalid JSON syntax - should print useful error message', async (flags) => {
      ...
      const jobFileContents = await (async () => readFileAsync(`${process.cwd()}/testResources/jobs/job_2.0.0-alpha.3_local_missing_comma`, { encoding: 'utf8' }))();
    ...
    it('- on - invalid job based on purpleteam schema - should print useful error message', async (flags) => {
      ...
      const { context: { rewiredApi, critStub } } = flags;
      const jobFileContents = await (async () => readFileAsync(`${process.cwd()}/testResources/jobs/job_2.0.0-alpha.3_local_missing_type_of_appScanner`, { encoding: 'utf8' }))();
