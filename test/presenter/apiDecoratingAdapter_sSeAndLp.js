...
test.serial('longPollTesterFeedback LP and handlers - given a mock event for each of the available testers sessions - given invocation of all the tester events - relevant handler instances should be run', async (t) => {
  ...
  const viewHandlerStats = {
    testerProgress: {
      ...
      params: [
        ...
        {
          matched: undefined,
          expected: { testerType: 'app', sessionId: 'lowPrivUser', message: 'response one', ptLogger: true }
        },
        {
          matched: undefined,
          expected: { testerType: 'app', sessionId: 'adminUser', message: 'response two', ptLogger: true }
        },
        {
          matched: undefined,
          expected: { testerType: 'server', sessionId: 'NA', message: 'response three', ptLogger: true }
        },
        {
          matched: undefined,
          expected: { testerType: 'tls', sessionId: 'NA', message: 'response four', ptLogger: true }
        }
      ]
    },
    testerPctComplete: {
  ...
  nock(apiUrl)
    .get(`/${TesterFeedbackRoutePrefix('lp')}/app/lowPrivUser`)
    .reply(200, [
      { data: { progress: 'response one' }, event: 'testerProgress', id: 1 },
      { data: { pctComplete: 8 }, event: 'testerPctComplete', id: 2 },
      { data: { bugCount: 3 }, event: 'testerBugCount', id: 3 }
    ])
    .get(`/${TesterFeedbackRoutePrefix('lp')}/app/lowPrivUser`)
    .reply(200, [
      { data: { progress: 'Tester finished' }, event: 'testerProgress', id: 4 }
    ])
    .get(`/${TesterFeedbackRoutePrefix('lp')}/app/adminUser`)
    .reply(200, [
      { data: { progress: 'response two' }, event: 'testerProgress', id: 5 },
      { data: { pctComplete: 99 }, event: 'testerPctComplete', id: 6 },
      { data: { bugCount: 7 }, event: 'testerBugCount', id: 7 }
    ])
    .get(`/${TesterFeedbackRoutePrefix('lp')}/app/adminUser`)
    .reply(200, [
      { data: { progress: 'Tester finished' }, event: 'testerProgress', id: 8 }
    ])
    .get(`/${TesterFeedbackRoutePrefix('lp')}/server/NA`)
    .reply(200, [
      { data: { progress: 'response three' }, event: 'testerProgress', id: 9 },
      { data: { pctComplete: 1 }, event: 'testerPctComplete', id: 10 },
      { data: { bugCount: 1 }, event: 'testerBugCount', id: 11 }
    ])
    .get(`/${TesterFeedbackRoutePrefix('lp')}/server/NA`)
    .reply(200, [
      { data: { progress: 'Tester finished' }, event: 'testerProgress', id: 12 }
    ])
    .get(`/${TesterFeedbackRoutePrefix('lp')}/tls/NA`)
    .reply(200, [
      { data: { progress: 'response four' }, event: 'testerProgress', id: 13 },
      { data: { pctComplete: 0 }, event: 'testerPctComplete', id: 14 },
      { data: { bugCount: 900 }, event: 'testerBugCount', id: 15 }
    ])
    .get(`/${TesterFeedbackRoutePrefix('lp')}/tls/NA`)
    .reply(200, [
      { data: { progress: 'Tester finished' }, event: 'testerProgress', id: 16 }
    ]);
